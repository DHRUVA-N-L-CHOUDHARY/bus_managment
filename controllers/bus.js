...
exports.createBus = async (req, res) => {
  try {
    ...
    if (
      !mongoose.Types.ObjectId.isValid(sourceStop) ||
      !mongoose.Types.ObjectId.isValid(destinationStop)
    ) {
      return res.status(400).json({
        success: false,
    ...
    const modelUpload = await cloudinary.uploader.upload(
      modelFile.tempFilePath,
      {
        folder: "bus_3d_models",
        resource_type: "raw", 
      }
    );
    ...
    let busDetailsInstance = await BusDetailsModel.create({
      ...
      threeDModel: {
        ...
        previewImagePublicId: previewUpload.public_id,
        uploadedAt: new Date(),
      },
    });
    ...
    let bus = await BusModel.findById(busInstance._id)
      .populate("sourceStop")
...
exports.updateThreeDModel = async (req, res) => {
    try {
        const { busDetailsId } = req.params;
        const { modelFile, previewImage } = req.files;

        if (!modelFile || !previewImage) {
            return res.status(400).json({
                success: false,
                message: "Both model file and preview image are required",
            });
        }

        const busDetails = await BusDetailsModel.findById(busDetailsId);
        if (!busDetails) {
            return res.status(404).json({
                success: false,
        ...
        if (busDetails.threeDModel.modelPublicId) {
            await cloudinary.uploader.destroy(busDetails.threeDModel.modelPublicId, { resource_type: "raw" });
        }
        if (busDetails.threeDModel.previewImagePublicId) {
            await cloudinary.uploader.destroy(busDetails.threeDModel.previewImagePublicId);
        }

        // Upload new 3D model file
        const modelUpload = await cloudinary.uploader.upload(modelFile.tempFilePath, {
            folder: "bus_3d_models",
            resource_type: "raw",
        });

        // Upload new preview image
        const previewUpload = await cloudinary.uploader.upload(previewImage.tempFilePath, {
            folder: "bus_preview_images",
        });

        // Update the document
        busDetails.threeDModel = {
            modelUrl: modelUpload.secure_url,
            modelPublicId: modelUpload.public_id,
            previewImageUrl: previewUpload.secure_url,
            previewImagePublicId: previewUpload.public_id,
        ...
        return res.status(200).json({
            ...
            threeDModel: busDetails.threeDModel,
        });
    } catch (error) {
        console.error(error);
        return res.status(500).json({
            success: false,
            message: "An error occurred while updating the 3D model",
            error: error.message,
        });
    }
};
